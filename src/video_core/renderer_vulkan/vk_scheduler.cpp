...
void LogSubmitDeviceLostDiagnostics(const Instance& instance) {
    ...
    for (const auto& checkpoint : instance.GetGraphicsQueue().getCheckpointDataNV()) {
        ...
        if (!command) {
            LOG_CRITICAL(Render_Vulkan,
                         "GPU submit checkpoint: stage={}, marker={:#x} (history expired)",
        ...
        LOG_CRITICAL(
            Render_Vulkan,
            "GPU submit checkpoint: stage={}, sequence={}, type={}, pipeline={:#018x}, "
            ...
            command->shader_hashes[0], command->shader_hashes[1], command->shader_hashes[2],
            command->shader_hashes[3], command->shader_hashes[4], command->shader_hashes[5],
...
void Scheduler::SubmitExecution(SubmitInfo& info) {
    ...
    if (submit_result == vk::Result::eErrorDeviceLost) {
        LogSubmitDeviceLostDiagnostics(instance);
    }
