...
layout (location = 1) out vec4 sampled_linear;
...
void main() {
    ...
    vec4 computed_color;
    if (pp.hdr) {
        computed_color = color_linear;
    } else {
        computed_color = vec4(gamma(color_linear.rgb), color_linear.a);
    }
    color = computed_color;
    sampled_linear = color_linear;
}
