...
TEST(PhysicalBackingPublicationCoordinator, AtomicallyMigratesDirtyOwnersIntoReplacementBuffer) {
    PhysicalBackingPublicationCoordinator coordinator{PhysicalBackingDeviceAddress{ImportedBase},
                                                      16 * PageSize};
    constexpr std::array spans{
        PhysicalBackingSpan{GuestA, PhysicalPage, PageSize, 7},
    ...
    ASSERT_TRUE(coordinator.MarkCachePageGpuDirty(first->token, 64, 128));
    ASSERT_TRUE(coordinator.MarkCachePageGpuDirty(second->token, 256, 512));
    constexpr u64 replacement_base = OverrideBase + 2 * PageSize;
    constexpr std::array rejected_requests{
        VideoCore::PhysicalBackingCachePageRequest{
            GuestA, PhysicalBackingDeviceAddress{replacement_base}},
        VideoCore::PhysicalBackingCachePageRequest{GuestB, PhysicalBackingDeviceAddress{}},
    };

    EXPECT_FALSE(coordinator.MigrateCachePagesForGuests(rejected_requests));
    EXPECT_EQ(coordinator.ResolveGuestPagePublication(GuestA)->value, OverrideBase);
    EXPECT_EQ(coordinator.ResolveGuestPagePublication(GuestB)->value, OverrideBase + PageSize);
    EXPECT_EQ(coordinator.ResolveCachePageDirtySlices(first->token),
              (std::vector<VideoCore::PhysicalBackingDirtySlice>{{64, 128}}));
    EXPECT_EQ(coordinator.ResolveCachePageDirtySlices(second->token),
    ...
    constexpr std::array requests{
        VideoCore::PhysicalBackingCachePageRequest{
            GuestA, PhysicalBackingDeviceAddress{replacement_base}},
        VideoCore::PhysicalBackingCachePageRequest{
            GuestB, PhysicalBackingDeviceAddress{replacement_base + PageSize}},
    };

    const auto migrated = coordinator.MigrateCachePagesForGuests(requests);

    ASSERT_TRUE(migrated.has_value());
    ASSERT_EQ(migrated->migrations.size(), 2);
    EXPECT_EQ(migrated->migrations[0].previous_token, first->token);
    EXPECT_EQ(migrated->migrations[1].previous_token, second->token);
    EXPECT_EQ(coordinator.ResolveGuestPagePublication(GuestA)->value, replacement_base);
    EXPECT_EQ(coordinator.ResolveGuestPagePublication(GuestB)->value,
              replacement_base + PageSize);
    EXPECT_FALSE(coordinator.ResolveCachePageDirtySlices(first->token));
    EXPECT_FALSE(coordinator.ResolveCachePageDirtySlices(second->token));
    EXPECT_EQ(coordinator.ResolveCachePageDirtySlices(migrated->migrations[0].token),
              (std::vector<VideoCore::PhysicalBackingDirtySlice>{{64, 128}}));
    EXPECT_EQ(coordinator.ResolveCachePageDirtySlices(migrated->migrations[1].token),
              (std::vector<VideoCore::PhysicalBackingDirtySlice>{{256, 512}}));
    EXPECT_FALSE(coordinator.RetireCachePageGpuDirty(first->token));
    EXPECT_FALSE(coordinator.RetireCachePageGpuDirty(second->token));
    const auto first_dirty =
        coordinator.RetireCachePageGpuDirty(migrated->migrations[0].token);
    const auto second_dirty =
        coordinator.RetireCachePageGpuDirty(migrated->migrations[1].token);
    ASSERT_TRUE(first_dirty.has_value());
    ASSERT_TRUE(second_dirty.has_value());
    EXPECT_EQ(first_dirty->dirty_slices,
              (std::vector<VideoCore::PhysicalBackingDirtySlice>{{64, 128}}));
    EXPECT_EQ(second_dirty->dirty_slices,
              (std::vector<VideoCore::PhysicalBackingDirtySlice>{{256, 512}}));
}
